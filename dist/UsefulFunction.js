export class UsefulFunction {
    static getIndexOfArrayMax(arrayOfNumbers) {
        if (!(Array.isArray(arrayOfNumbers)))
            throw Error("arrayOfNumbers is not an array");
        if (arrayOfNumbers.length == 0)
            throw Error("arrayOfNumbers contains no elements");
        let indexOfMax = 0;
    ...
    static incrementMap(map, key) {
        if (!map.has(key))
            throw new Error("Supplied map is bad");
    ...
    static getCounterClockwiseMove(hashGraph, currentNode) {
        ...
        for (let i = possibleNextMoves.length - 1; i >= 0; i--) {
            if (!this.doesMoveExistElsewhere(hashGraph, currentNode, possibleNextMoves[i]))
                continue;
    ...
    static isNodeSurroundedByNodes(hashGraph, nodeToCheck) {
        if (!(hashGraph.has(nodeToCheck)))
            throw Error("Key not found in hash graph");
    ...
    static getOutsideNode(hashGraph) {
        for (const key of hashGraph.keys()) {
            if (this.isNodeSurroundedByNodes(hashGraph, key))
                continue;
            return key;
        }
        return -1;
    ...
    static divideArray(array, number) {
        if (number == 0)
            throw new Error("Divisor is 0, critical failure, cannot divide by zero!");
        let newArray = [];
        for (const element of array) {
            newArray.push(element / number);
        }
...
// source - chatgpt
UsefulFunction.noDuplicates = (arr) => [...new Set(arr)];
